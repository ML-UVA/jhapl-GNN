...
def plot_both_sweeps():
    ...
    # Recursively hunt through all folders
    for root, dirs, files in os.walk(base_dir):
        for filename in files:
            ...
            if filename.startswith("metrics_") and filename.endswith(".json"):
                ...
                # Filter 1: Grab the NEW Version 2 MLP Decoder ADP metrics
                # We check the 'root' path to ensure it's pulling from your new folders
                if "adp" in graph_type.lower() and "mlp_decoder" in root.lower():
    ...
    print(f" -> Found {len(adp_results)} MLP Decoder ADP metrics.")
    ...
    if adp_results:
        ...
        plt.plot(thresholds, aucs, marker='s', linewidth=3, markersize=8, 
                 color='#d2691e', label='Version 2: ADP + MLP Decoder')
        
    plt.title("The Showdown: Euclidean Context vs. Physical Surface Area", fontsize=16, fontweight='bold')
    ...
    out_path = "./final_mlp_sweep_trend.png"
    plt.tight_layout()
