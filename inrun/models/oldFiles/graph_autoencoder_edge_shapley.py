...
data = load_neuron_graph(args.csv_path, args.feature_path)
...
def edge_recon_loss_per_edge(z, edge_index):
    src, dst = edge_index
    logits = (z[src] * z[dst]).sum(dim=1)
    return F.binary_cross_entropy_with_logits(
        logits,
        torch.ones_like(logits),
        reduction='none'  # <-- critical
    )
...
def train():
    ...
    edge_losses = edge_recon_loss_per_edge(
        z, train_data.pos_edge_label_index
    )
    edge_loss = edge_losses.mean()

    x_recon = feature_decoder(z)
    loss_per_node = torch.mean((x_recon - train_data.x) ** 2, dim=1)
    feature_loss = loss_per_node.mean()
    ...
    return float(loss), edge_losses.detach()
