"""
inrun package entry point.

Run from the repository root:

    python -m inrun --use_existing
...
def _ensure_synapses_pt(path, config_path):
    if os.path.exists(path):
        return
    print(f"[inrun] {path} not found — regenerating from raw .pbz2 exports...")
...
def _ensure_features_pt(path, synapses_path, positions_path):
    if os.path.exists(path):
        return
    print(f"[inrun] {path} not found — regenerating from {synapses_path} + {positions_path}...")
