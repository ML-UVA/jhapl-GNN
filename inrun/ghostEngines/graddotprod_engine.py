...
class GradDotProdEngine:
    ...
    def _aggregate_and_log_dot_products(self):
        ...
        if total_dot_product_iter is not None:
            ...
            if hasattr(self, 'edge_index'):
                info_this_iter['edge_index'] = to_device(self.edge_index, 'cpu')
    ...
    """def attach_and_store_valset(self, X_val, val_edge_index):
    ...
    def attach_and_store_valset(self, X_val, val_edge_index):
        ...
        torch.save(
            {
                'X_val': to_device(X_val, 'cpu'),
                'val_edge_index': to_device(val_edge_index, 'cpu'),
            },
            valset_path
        )
        print(f"[INFO] Saved validation set to {valset_path}")    

    def attach_train_batch(self, X_train, iter_num, batch_idx=None, edge_index=None):
        self.X_train = X_train
        self.iter_num = iter_num
        self.batch_idx = batch_idx
        if edge_index is not None:
            self.edge_index = edge_index.detach()
        elif hasattr(self, 'edge_index'):
            del self.edge_index
