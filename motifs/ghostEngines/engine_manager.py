...
class GhostEngineManager:
    ...
    def attach_train_batch(self, X_train, Y_train, iter_num, batch_idx=None, edge_index=None):
        """Attach training batch information to the engine (if applicable)."""
        if self.engine and hasattr(self.engine, 'attach_train_batch'):
            self.engine.attach_train_batch(X_train, Y_train, iter_num, batch_idx, edge_index=edge_index)
